...
class UserProfileStore:
    ...
    async def track_interaction(
        ...
        profile.avg_query_length = (
            (1 - alpha) * profile.avg_query_length + alpha * query_length
        )
        ...
        profile.typical_session_duration = (
            (1 - alpha) * profile.typical_session_duration + alpha * session_duration
        )
        
        if escalated:
            profile.escalation_rate = (
                (1 - alpha) * profile.escalation_rate + alpha * 1.0
            )
        else:
            profile.escalation_rate = (
                (1 - alpha) * profile.escalation_rate + alpha * 0.0
            )
        ...
        if satisfaction_score is not None:
            ...
            profile.avg_satisfaction_score = (
                (1 - alpha) * profile.avg_satisfaction_score 
